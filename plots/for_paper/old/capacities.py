# -*- coding: utf-8 -*-
"""
Created on Tue Aug  5 18:40:32 2025
...
def get_production(network, tech, target_region):
    ...
    total_mwh = p1[selected_links.index]
...
def share_green_h2(n):
    ...
    h2_clean_df = n.links.p_nom_opt.loc[h2_clean]
    h2_dirty_df = n.links.p_nom_opt.loc[h2_dirty]
...
def share_bio_naphtha(n):
    ...
    oil_bio_total = n.links.p_nom_opt[oil_biomass].sum()
    oil_ref_total = n.generators.p_nom_opt[oil_ref].sum()

    # FT flows with country info
    oil_ft_df = n.links.p_nom_opt[oil_ft]
...
def get_steel_prod_eu(n):
    ...
    efficiency_eaf = n.links.efficiency.loc[eaf_links].iloc[0]
    efficiency_bof = n.links.efficiency.loc[bof_links].iloc[0]
    
    steel_eaf = n.links.p_nom_opt.loc[eaf_links] * efficiency_eaf * 8760 / timestep
    steel_bof = n.links.p_nom_opt.loc[bof_links] * efficiency_bof *  8760 / timestep
    ...
    uncaptured = n.links.p_nom_opt[n.links.p_nom_opt.index.str.contains('steel BOF process emis to atmosphere')].sum().sum()
    captured = n.links.p_nom_opt[n.links.p_nom_opt.index.str.contains('steel BOF CC')].sum().sum()
    ...
    dri_ch4 = n.links.p_nom_opt[n.links.p_nom_opt.index.str.contains('CH4 to syn gas DRI')] * 8760 / timestep
    dri_h2 = n.links.p_nom_opt[n.links.p_nom_opt.index.str.contains('H2 to syn gas DRI')] *8760 / timestep
...
def get_cement_prod_eu(n):
    ...
    efficiency = n.links.efficiency[cement_links.index].iloc[0]
    total_cement_mwh = n.links.p_nom_opt[cement_links.index].sum() * efficiency * 8760 / timestep

    # Emissions not captured and captured by CCS
    uncaptured = n.links.p_nom_opt[n.links.p_nom_opt.index.str.contains('cement process emis to atmosphere')].sum().sum() 
    captured = n.links.p_nom_opt[n.links.p_nom_opt.index.str.contains('cement TGR')].sum().sum()
...
def get_ammonia_prod_eu(n):
    ...
    efficiency = n.links.efficiency[ammonia_links.index].iloc[0]
    total_p1 = p1[ammonia_links.index]
    total_mwh = total_p1.sum() * efficiency * 8760 / timestep
...
def get_methanol_prod_eu(n):
    ...
    efficiency = n.links.efficiency[methanol_links.index].iloc[0]
    total_p1 = p1[methanol_links.index]
    total_mwh = total_p1.sum() * efficiency * 8760 / timestep
...
def get_hvc_prod_eu(n):
    ...
    efficiency = n.links.efficiency[hvc_links.index[hvc_links.index.str.contains('naphtha')]].iloc[0]
    hvc_naphtha = n.links.p_nom_opt[hvc_links.index[hvc_links.index.str.contains('naphtha')]].sum() * efficiency * 8760 / timestep
...
def get_h2_prod_eu(n):
    ...
    for tech in tech_names:
        ...
        # Multiply installed capacity by efficiency, then sum
        efficiency = n.links.efficiency[tech_links].iloc[0]
        prod = n.links.p_nom_opt[tech_links].sum() * efficiency * 8760 / timestep # in MWh

        # Convert MWh → tH2 → MtH2
        tech_productions[tech] = prod / lhv_hydrogen / 1e6
    ...
    for i, tech in enumerate(technologies):  # tech index = row
        for j, scenario in enumerate(scenarios):  # scenario index = col
            ...
            if j == 0:
                ...
                ax.set_ylabel(f"{title} [Mt/a]", fontsize=12)
            else:
...
plot_total_eu_production_by_tech_reversed(
    ...
    years=years,
    save_path="graphs/european_capacities_stacked.png"
)
