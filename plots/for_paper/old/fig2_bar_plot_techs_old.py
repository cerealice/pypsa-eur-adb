# -*- coding: utf-8 -*-
"""
Created on Wed Jun  4 16:49:54 2025
...
def plot_total_eu_production_by_tech_reversed(
    ...
    if n_cols >= 3:
        ...
        y_bottom = axes[-1, 0].get_position().y0  # bottom of last row
    
        fig.lines.append(
            plt.Line2D([x_line, x_line], [y_bottom, y_top], transform=fig.transFigure, color='black', linewidth=1)
        )
        
    # Horizontal line between rows 5 and 6
    if n_rows >= 6:
        ax_row5 = axes[4, 0]
        ax_row6 = axes[5, 0]
    
        y5 = ax_row5.get_position().y0
        y6 = ax_row6.get_position().y1
        y_line = (y5 + y6) / 2
    
        # Define horizontal extent using leftmost and rightmost axes
        x_left = axes[0, 0].get_position().x0
        x_right = axes[0, -1].get_position().x1
    
        fig.lines.append(
            plt.Line2D([x_left, x_right], [y_line, y_line], transform=fig.transFigure, color='black', linewidth=3)
        )
    ...
    if n_cols >= 4:
        ...
        fig.text(x_first_group, 0.92, "No Climate Policy", ha='center', fontsize=14)
        fig.text(x_second_group, 0.92, "Climate Policy", ha='center', fontsize=14)
...
nice_scenario_names = {
    "base_reg_deindustrial": "Deindustrialization",
    "policy_reg_deindustrial": "Deindustrialization",
    "base_reg_regain": "Reindustrialization",
...
plot_total_eu_production_by_tech_reversed(
    ...
    years=years,
    save_path="graphs/european_production_stacked_h2separated.png"
)
