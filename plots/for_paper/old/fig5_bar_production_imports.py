# -*- coding: utf-8 -*-
"""
Created on Sun Jul 20 14:54:58 2025
...
def get_steel_prod_eu_import(network):
    ...
    import_hbi = network.generators_t.p.loc[:, network.generators_t.p.columns.str.contains('HBI import')].sum().sum() * timestep
    ...
    ch4_eaf = dri_eaf * share_ch4 / 1e3

    # H2 splits by origin
    green_h2_eaf = dri_eaf * share_h2 * shares["Green El"].mean() / 1e3
    grey_el_h2_eaf = dri_eaf * share_h2 * shares["Grey El"].mean() / 1e3
    blue_h2_eaf = dri_eaf * share_h2 * shares["Blue"].mean() / 1e3
    grey_h2_eaf = dri_eaf * share_h2 * shares["Grey"].mean() / 1e3
...
scenario_labels = {
    ...
    "import_policy_reg_deindustrial": "Deindustrialization\nIntermediate Imports",
...
for scenario in scenarios:
    for year in years[1:]:
        ...
        price_data["methanol"].loc[scenario, year] = weighted_average_marginal_price(n, keyword='methanol', exclude_labels=["import"]) * lhv_methanol
...
# === SUMMED SYSTEM COST OVER 2030–2050 (each decade × 10 years) ===
summed_costs = {
    scenario: sum(data_dict["Annual system cost [bnEUR/a]"][scenario][year] * 10 for year in years)
    for scenario in scenarios
}
...
summed_costs_df = pd.DataFrame({
    "Scenario": [scenario_labels[s] for s in scenarios],
    "Total System Cost (2020–2050, b€)": [summed_costs[s] for s in scenarios],
    "Color": [scenario_colors[s] for s in scenarios]
})

# === PLOT TOTAL COST AS BARS ===
plt.figure(figsize=(10, 6))
bars = plt.bar(
    x=summed_costs_df["Scenario"],
    height=summed_costs_df["Total System Cost (2020–2050, b€)"],
    color=summed_costs_df["Color"]
)
plt.ylabel("Total System Cost [b€]")
plt.title("Cumulative System Cost (2030–2050)")
plt.xticks(rotation=15, ha='right')
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()

# Save and show
plt.savefig("./graphs/summed_costs_import.png", dpi=300)
plt.show()
...
average_costs_df = pd.DataFrame({
    "Scenario": [scenario_labels[s] for s in scenarios],
    "Average System Cost (2030–2050, b€)": [average_costs[s] for s in scenarios],
    "Color": [scenario_colors[s] for s in scenarios]
})

# === PLOT AVERAGE COST AS BARS ===
plt.figure(figsize=(10, 6))
bars = plt.bar(
    x=average_costs_df["Scenario"],
    height=average_costs_df["Average System Cost (2030–2050, b€)"],
    color=average_costs_df["Color"]
)
plt.ylabel("Average Annual System Cost [b€]")
plt.title("Average Annual System Cost (2030–2050)")
plt.xticks(rotation=15, ha='right')
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()

# Save and show
plt.savefig("./graphs/average_costs_import.png", dpi=300)
plt.show()
