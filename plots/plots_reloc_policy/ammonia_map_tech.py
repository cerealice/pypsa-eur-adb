# -*- coding: utf-8 -*-
"""
Created on Thu May  8 12:20:36 2025
...
def plot_ammonia_map(n, regions, year, i,j, max_ammonia,ax=None):
    ...
    ammonia_prod = -n.links_t.p1.loc[:, ammonia_links.index].sum() * timestep / lhv_ammonia  # t ammonia
    ...
    regions["ammonia"] = ammonia_prod_df.ammonia_prod / 1e3  # Convert to kt
...
for scenario in scenarios:
    for year in years:
        fn = root_dir + f"results_8h/{scenario}/networks/base_s_39___{year}.nc"
        n = pypsa.Network(fn)
        ...
        ammonia_by_region = (
            ...
            .sum()
            .div(1e3) * timestep /lhv_ammonia # convert to kt ammonia/yr
        )
...
plt.savefig("graphs/ammonia_prod_per_country_pie_chart_eu_dem.png", bbox_inches='tight')
plt.show()
