...
def add_brownfield(
    ...
    for c in n_p.iterate_components(["Link", "Generator", "Store"]):
        ...
        n_p.remove(c.name, c.static.index[c.static.build_year + c.static.lifetime <= year])

        # remove assets if their optimized nominal capacity is lower than a threshold
        # since CHP heat Link is proportional to CHP electric Link, make sure threshold is compatible
        chp_heat = c.static.index[
            (c.static[f"{attr}_nom_extendable"] & c.static.index.str.contains("urban central"))
        ...
        if not chp_heat.empty:
            ...
            n_p.remove(
                c.name,
                chp_heat[c.static.loc[chp_heat, f"{attr}_nom_opt"] < threshold_chp_heat],
            )
